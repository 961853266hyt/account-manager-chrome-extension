...
import {
  Settings2, Plus, MoreHorizontal, RefreshCw, Pencil, Trash2, ArrowLeftRight, Globe,
} from 'lucide-react'
...
export default function App() {
  ...
  const [matching, setMatching] = useState([]) // 与当前主机匹配的已存作用域
  ...
  return (
    <div className="flex max-h-[580px] min-h-[360px] flex-col bg-background text-foreground">
      {/* 头部：站点上下文 */}
              ...
              return (
                <li
                  ...
                  className={`flex items-center gap-2.5 rounded-lg border p-2.5 transition-colors ${
                    active ? 'border-primary/60 bg-accent/60' : 'hover:bg-accent/40'
                  }`}
                >
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-primary font-semibold text-[12px] text-primary-foreground">
                    {acc.name.slice(0, 2).toUpperCase()}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="flex items-center gap-1.5 truncate text-[13px] font-medium leading-tight">
                      <span className="truncate">{acc.name}</span>
                      {active && <Badge className="h-4 px-1.5 text-[10px]">当前</Badge>}
                    </p>
                  ...
                  <Button
                    ...
                    title={active ? '重新写入该账号的 Cookie' : '切换到该账号并刷新页面'}
                  >
                    <ArrowLeftRight /> 切换
                  </Button>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon-sm" disabled={busy} title="更多操作">
                        <MoreHorizontal />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onSelect={() => handleUpdate(acc)}>
                        <RefreshCw /> 更新为当前登录态
                      </DropdownMenuItem>
                      <DropdownMenuItem onSelect={() => setRenameTarget(acc)}>
                        <Pencil /> 重命名
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        className="text-destructive focus:text-destructive"
                        onSelect={() => handleDelete(acc)}
                      >
                        <Trash2 /> 删除
                      </DropdownMenuItem>
                    </DropdownMenuContent>
